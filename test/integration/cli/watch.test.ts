...
const waitFor = (condition, timeoutInMs) =>
  new Promise((resolve, reject) => {
    ...
    const run = () => {
      ...
      if (result !== false && error === null) {
        ...
        reject(
          new Error(`Condition not met within time: ${condition.toString()}`)
        )
      }
    }
    setTimeout(run, timeoutDelay())
...
xdescribe('cli --watch', function() {
  ...
  it('should log syntax error and wait until that is fixed before running tests', function() {
    ...
    return Promise.resolve() // wait until the output matches our condition
      .then(() =>
        waitFor(() => assert.include(mw.log, 'Unexpected token'), 5000)
      ...
      .then(updatedTestId =>
        waitFor(
          () =>
            assert.include(mw.log, updatedTestId) &&
  ...
  it('should catch other errors outside of tests', function() {
    ...
    return Promise.resolve() // wait until the output matches our condition
      .then(() =>
        waitFor(() => assert.include(mw.log, `Error ${testFile}`), 5000)
      ) // output matched our condition
      .then(() => {
  ...
  it('should catch uncaught errors that occur after tests are done', function() {
    ...
    return Promise.resolve() // wait until the output matches our condition
      .then(() =>
        waitFor(() => assert.include(mw.log, 'UNCAUGHT EXCEPTION'), 5000)
      ) // output matched our condition
      .then(() => {
  ...
  it('should abort test suite when a file changes while running tests and then test again', function() {
    ...
    return Promise.resolve() // wait until the first async test start
      .then(() =>
        waitFor(() => assert.include(mw.log, `starting ${testId} - 1`), 5000)
      )
      .then(() => {
  ...
  it('should also abort tests that will never finish (e.g. by mistake) when timeouts are disabled and run tests again', function() {
    ...
    return Promise.resolve() // wait until the first async test start
      .then(() =>
        waitFor(() => assert.include(mw.log, `starting ${testId}`), 5000)
      )
      .then(() => {
        // clear log to receive only changes
        mw.clearLog()
      ...
      .then(() =>
        waitFor(
          () =>
            ...
            assert.include(mw.log, '2 passing'),
          5000
        )
      )
      .then(() => {
