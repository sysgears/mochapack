...
xdescribe('cli --watch', function () {
  ...
  it('should log syntax error and wait until that is fixed before running tests', function () {
    ...
    .then(() => waitFor(() => assert.include(mw.log, 'Unexpected token'), 5000)) // output matched our condition
    .then(() => {
    ...
    .then(updatedTestId => waitFor(() => assert.include(mw.log, updatedTestId) && assert.include(mw.log, '1 passing'), 5000)) // output matched our condition
    .then(() => {
  ...
  it('should catch other errors outside of tests', function () {
    ...
    return Promise.resolve() // wait until the output matches our condition
    .then(() => waitFor(() => assert.include(mw.log, `Error ${testFile}`), 5000)) // output matched our condition
    .then(() => {
  ...
  it('should catch uncaught errors that occur after tests are done', function () {
    ...
    return Promise.resolve() // wait until the output matches our condition
    .then(() => waitFor(() => assert.include(mw.log, 'UNCAUGHT EXCEPTION'), 5000)) // output matched our condition
    .then(() => {
  ...
  it('should run a test', function () {
    ...
    return Promise.resolve() // wait until the output matches our condition
    .then(() => waitFor(() => assert.include(mw.log, testId) && assert.include(mw.log, '1 passing'), 5000)).catch(e => e).then(e => {
      // finally, kill watch process
      mw.kill();
      // maybe rethrow error
      assert.ifError(e);
  ...
  it('should run a test again when it changes', function () {
    ...
    return Promise.resolve() // wait until the output matches our condition
    .then(() => waitFor(() => assert.include(mw.log, testId) && assert.include(mw.log, '1 passing'), 5000)) // output matched our condition
    .then(() => {
      // clear log to receive only changes
      mw.clearLog();
  ...
  it('should abort test suite when a file changes while running tests and then test again', function () {
    ...
    .then(() => waitFor(() => assert.include(mw.log, `starting ${testId} - 1`), 5000)).then(() => {
    ...
    .then(() => waitFor(() => assert.include(mw.log, '0 passing'), 5000)).then(() => {
      // check if tests were aborted
      assert.notInclude(mw.log, `finished ${testId} - 2`);
    }) // wait until tests were tested again
    .then(() => waitFor(() => assert.include(mw.log, updatedTestId) && assert.include(mw.log, '1 passing'), 5000)).catch(e => e).then(e => {
      // finally, kill watch process
      mw.kill();
      // maybe rethrow error
      assert.ifError(e);
  ...
  it('should also abort tests that will never finish (e.g. by mistake) when timeouts are disabled and run tests again', function () {
    ...
    return Promise.resolve() // wait until the first async test start
    .then(() => waitFor(() => assert.include(mw.log, `starting ${testId}`), 5000)).then(() => {
      // clear log to receive only changes
      mw.clearLog();
    ...
    .then(() => waitFor(() => assert.include(mw.log, 'Tests aborted'), 5000)).then(() => {
      // check if tests were aborted
      assert.notInclude(mw.log, `finished ${testId} - 2`);
    }) // wait until tests were tested again
    .then(() => waitFor(() => assert.include(mw.log, updatedTestId) && assert.include(mw.log, '1 passing'), 5000)).catch(e => e).then(e => {
  ...
  it('should recognize new test entries that match the pattern', function () {
    ...
    return Promise.resolve() // wait until the output matches our condition
    .then(() => waitFor(() => assert.include(mw.log, testId1) && assert.include(mw.log, '1 passing'), 5000)) // output matched our condition
    .then(() => {
      // clear log to receive only changes
      mw.clearLog();
      // create new test
    ...
    .then(() => waitFor(() => assert.include(mw.log, testId2) && assert.include(mw.log, '1 passing'), 5000)).then(() => {
      assert.notInclude(mw.log, testId1);
      assert.notInclude(mw.log, testFile1);
  ...
  it('should recognize multiple new test entries that match the pattern', function () {
    ...
    .then(() => waitFor(() => assert.include(mw.log, testId2) && assert.include(mw.log, testId3) && assert.include(mw.log, '2 passing'), 5000)).then(() => {
      assert.notInclude(mw.log, testId1);
      assert.notInclude(mw.log, testFile1);
    }) // output matched our condition
    .catch(e => e).then(e => {
  ...
  it('should recognize deleted test entries that match the pattern', function () {
    ...
    .then(() => waitFor(() => assert.include(mw.log, 'passing'), 5000)).then(() => {
