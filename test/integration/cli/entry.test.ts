...
const fixtureDirTmp = path.relative(
  process.cwd(),
  path.join(__dirname, 'fixtureTmp')
)
...
describe('cli - entry', function() {
  context('single test file as option', function() {
    before(function() {
      this.passingTest = normalizePath(
        path.join(fixtureDirTmp, 'passing-test.js')
      )
      this.failingTest = normalizePath(
        path.join(fixtureDirTmp, 'failing-test.js')
      )
      this.corruptedTest = normalizePath(
        path.join(fixtureDirTmp, 'corrupted-test.js')
      )
      this.runtimeErrorTest = normalizePath(
        path.join(fixtureDirTmp, 'runtime-error-test.js')
      )
    ...
    it('handles failed module with syntax errors', function(done) {
      exec(
        `node ${binPath} --mode development "${this.corruptedTest}"`,
        err => {
    ...
    it('handles module with runtime errors', function(done) {
      exec(
        `node ${binPath} --mode development "${this.runtimeErrorTest}"`,
        err => {
          assert.isNotNull(err)
          assert.isAbove(err.code, 0)
          done()
    ...
    it('runs successfull test', function(done) {
      exec(
        `node ${binPath} --mode development "${this.passingTest}"`,
        (err, output) => {
          assert.isNull(err)
          assert.include(output, this.passingTest)
    ...
    it('runs failing test', function(done) {
      exec(
        `node ${binPath} --mode development "${this.failingTest}"`,
        (err, output) => {
          assert.isNotNull(err)
          assert.strictEqual(err.code, 1)
          assert.include(output, this.failingTest)
          assert.include(output, '0 passing')
  ...
  context('multiple test files as option', function() {
    before(function() {
      ...
      this.passingTest2 = normalizePath(
        path.join(fixtureDirTmp, 'passing-test2.js')
      )
      this.failingTest = normalizePath(
        path.join(fixtureDirTmp, 'failing-test.js')
      )
      this.failingTest2 = normalizePath(
        path.join(fixtureDirTmp, 'failing-test2.js')
      )
      this.corruptedTest = normalizePath(
        path.join(fixtureDirTmp, 'corrupted-test.js')
      )
      this.corruptedTest2 = normalizePath(
        path.join(fixtureDirTmp, 'corrupted-test2.js')
      )
      createTest(this.passingTest, true)
    ...
    it('handles failed module with syntax errors', function(done) {
      exec(
        `node ${binPath} --mode development "${this.corruptedTest}" "${this.corruptedTest2}"`,
        err => {
          assert.isNotNull(err)
          assert.isAbove(err.code, 0)
          done()
    ...
    it('runs successfull test', function(done) {
      exec(
        `node ${binPath} --mode development "${this.passingTest}" "${this.passingTest2}"`,
        (err, output) => {
          assert.isNull(err)
          assert.include(output, this.passingTest)
          assert.include(output, this.passingTest2)
    ...
    it('runs failing test', function(done) {
      exec(
        `node ${binPath} --mode development "${this.failingTest}" "${this.failingTest2}"`,
        (err, output) => {
          assert.isNotNull(err)
          assert.strictEqual(err.code, 2)
          assert.include(output, this.failingTest)
          assert.include(output, this.failingTest2)
          assert.include(output, '0 passing')
          assert.include(output, '2 failing')
          done()
        }
      )
    })
  ...
  context('glob pattern as option', function() {
    ...
    const corruptedPatterns = [path.join(fixtureDirTmp, 'corrupted-*.js')].map(
      normalizePath
    )
    ...
    passingPatterns.forEach(pattern => {
      ...
      it(`runs ${files.length} passing tests of ${testFiles.length} with pattern '${pattern}'`, function(done) {
        exec(
          `node ${binPath} --mode development "${pattern}"`,
          (err, output) => {
            assert.isNull(err)
            files.forEach(file => {
              assert.include(output, file)
            })
            assert.include(output, `${files.length} passing`)
            done()
          }
        )
      })
      ...
      it(`runs ${files.length} failing tests of ${testFiles.length} with pattern '${pattern}'`, function(done) {
        exec(
          ...
          (err, output) => {
            ...
            assert.include(output, '0 passing')
            assert.include(output, `${files.length} failing`)
            done()
          }
        )
      })
    ...
    it(`runs ${files.length} passing tests of ${testFiles.length} with pattern '${pattern}'`, function(done) {
      exec(`node ${binPath} --mode development ${pattern}`, (err, output) => {
        assert.isNull(err)
        files.forEach(file => {
          assert.include(output, file)
        })
  ...
  context('directory as option', function() {
    ...
    context('directory with passing tests', function() {
      ...
      it("runs all tests in directory'", function(done) {
        ...
        exec(
          `node ${binPath} --mode development "${fixtureDirTmp}"`,
          (err, output) => {
            assert.isNull(err)
            files.forEach(file => {
              assert.include(output, file)
      ...
      it("runs all tests matching file glob'", function(done) {
        ...
        const files = this.testFiles.filter(matcher)
        exec(
          `node ${binPath} --mode development --glob "*-test-3.js" "${fixtureDirTmp}"`,
          (err, output) => {
            assert.isNull(err)
            files.forEach(file => {
              assert.include(output, file)
      ...
      it("runs all tests in directory & subdirectories'", function(done) {
        ...
        exec(
          `node ${binPath} --mode development --recursive "${fixtureDirTmp}"`,
          (err, output) => {
            assert.isNull(err)
            files.forEach(file => {
              assert.include(output, file)
  ...
  context(
    'respect file extensions in webpack config via resolve.extensions for directory entries',
    function() {
      before(function() {
        this.index = 0
      ...
      it('resolve.extensions will be used for module resolution when no --glob is given', function(done) {
        exec(
          `node ${binPath} --mode development --webpack-config "${this.configPath}" "${this.testDir}"`,
          (err, output) => {
            assert.isNull(err)
            ...
            this.ignoredFiles.forEach(file => {
              assert.notInclude(output, file)
            })
            assert.include(output, `${this.testFiles.length} passing`)
            done()
          }
        )
      })
      ...
      it('resolve.extensions will not be used for module resolution when --glob is given', function(done) {
        ...
        const files = this.testFiles.filter(matcher)
        exec(
          `node ${binPath} --mode development --webpack-config "${this.configPath}" --glob "*.js" "${this.testDir}"`,
          (err, output) => {
            assert.isNull(err)
            files.forEach(file => {
              assert.include(output, file)
