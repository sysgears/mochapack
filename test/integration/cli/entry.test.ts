...
describe('cli - entry', function () {
  context('single test file as option', function () {
    ...
    it('runs successfull test', function (done) {
      exec(`node ${binPath} --mode development "${this.passingTest}"`, (err, output) => {
        assert.isNull(err);
    ...
    it('runs failing test', function (done) {
      exec(`node ${binPath} --mode development "${this.failingTest}"`, (err, output) => {
        assert.isNotNull(err);
  ...
  context('multiple test files as option', function () {
    ...
    it('runs successfull test', function (done) {
      exec(`node ${binPath} --mode development "${this.passingTest}" "${this.passingTest2}"`, (err, output) => {
        assert.isNull(err);
        assert.include(output, this.passingTest);
    ...
    it('runs failing test', function (done) {
      exec(`node ${binPath} --mode development "${this.failingTest}" "${this.failingTest2}"`, (err, output) => {
        assert.isNotNull(err);
  ...
  context('glob pattern as option', function () {
    ...
    const corruptedPatterns = [path.join(fixtureDirTmp, 'corrupted-*.js')].map(normalizePath);
    ...
    const passingPatterns = [path.join(fixtureDirTmp, 'passing-*.js'), path.join(fixtureDirTmp, 'passing-*-1.js'), path.join(fixtureDirTmp, '**/passing-*.js')].map(normalizePath);
    ...
    const failingPatterns = [path.join(fixtureDirTmp, 'failing-*.js'), path.join(fixtureDirTmp, 'failing-*-7.js'), path.join(fixtureDirTmp, 'failing-*-@(5|6).js')].map(normalizePath);
    ...
    const multiPassingPatterns = [path.join(fixtureDirTmp, 'passing-*-1.js'), path.join(fixtureDirTmp, 'passing-*-2.js'), path.join(fixtureDirTmp, 'passing-*-3.js')].map(normalizePath);
  ...
  context('respect file extensions in webpack config via resolve.extensions for directory entries', function () {
    before(function () {
      ...
      this.testFiles = ['ts', 'tsx', 'js', 'jsx'].map(ext => {
        const file = path.join(this.testDir, `passing-test-${this.index}.${ext}`);
        this.index += 1;
        return file;
      }).map(file => normalizePath(file));

      this.ignoredFiles = ['coffee'].map(ext => {
        const file = path.join(this.testDir, `passing-test-${this.index}.${ext}`);
        this.index += 1;
        return file;
      }).map(file => normalizePath(file));
    ...
    it('resolve.extensions will be used for module resolution when no --glob is given', function (done) {
      exec(`node ${binPath} --mode development --webpack-config "${this.configPath}" "${this.testDir}"`, (err, output) => {
        assert.isNull(err);
