...
describe('cli --webpack-config', function() {
  ...
  it('throws when not found', function(done) {
    ...
    exec(command, (err, output) => {
      assert.include(
        output,
        `Webpack config could not be found: ${configNotFound}`
      )
      done()
    })
  })
  ...
  it('passes --webpack-env random to config', function(done) {
    ...
    exec(
      `node ${binPath} --webpack-config ${config} --webpack-env ${env} "${testSimple}"`,
      (err, output) => {
        assert.isNull(err)
  ...
  it('passes --webpack-env object to config', function(done) {
    ...
    const env = Math.random()
    exec(
      `node ${binPath} --webpack-config ${config} --webpack-env.test ${env} "${testSimple}"`,
      (err, output) => {
        assert.isNull(err)
