...
const tests = fs
  .readdirSync(base)
  .filter(testName => fs.existsSync(path.join(base, testName, 'entry.js')))
...
describe('statsFormatter', function() {
  ...
  tests.forEach(testName => {
    ...
    const ensureConsistentCompare = _.flow([
      ...
      message =>
        message.replace(new RegExp(testDirPath, 'g'), `Xdir/${testName}`),
      message => message.replace(new RegExp(process.cwd(), 'g'), 'Cdir')
    ])

    it(`should print correct stats for ${path.basename(
      testDirPath
    )}`, function(done) {
      const formatter = createStatsFormatter(testDirPath)
      assert.isFunction(
        formatter,
        'createStatsFormatter should return a function'
      )
      ...
      compiler.run((err, stats) => {
        ...
        assert.isArray(
          warnings,
          'statsFormatter should return an Array of warnings'
        )
        assert.lengthOf(
          warnings,
          stats.compilation.warnings.length,
          'Length of warnings should match original length'
        )
        assert.isArray(
          errors,
          'statsFormatter should return an Array of errors'
        )
        assert.lengthOf(
          errors,
          stats.compilation.errors.length,
          'Length of errors should match original length'
        )
        ...
        if (path.basename(testDirPath) === 'babel-loader-syntax-error') {
          const nodeMajorVersion = process.version
            .split('.')[0]
            .replace('v', '')
          fixtureNodeId =
            parseInt(nodeMajorVersion, 10) <= 11
              ? ''
              : `.node-${nodeMajorVersion}`
        }
        const expectedWarningsPath = path.join(
          testDirPath,
          `warnings.webpack-${webpackMajorVersion}.txt`
        )
        ...
        if (
          !fs.existsSync(expectedWarningsPath) ||
          ...
          fs.outputFileSync(expectedErrorsPath, errorsContent)
            ...
            new Error(
              'Expected files did not exist, yet. Created it. Have a look at them'
            )
          )
          return
        }

        const expectedWarningsContent = ensureConsistentCompare(
          fs.readFileSync(expectedWarningsPath, 'utf-8')
        )
        const expectedErrorsContent = ensureConsistentCompare(
          fs.readFileSync(expectedErrorsPath, 'utf-8')
        )

        assert.strictEqual(
          warningsContent,
          expectedWarningsContent,
          'Warnings should match'
        )
        assert.strictEqual(
          errorsContent,
          expectedErrorsContent,
          'Errors should match'
        )
