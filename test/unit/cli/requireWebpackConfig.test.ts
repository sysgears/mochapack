...
describe('requireWebpackConfig', () => {
  ...
  ;(process.platform === 'win32' ? it.skip : it)(
    'requires symlinked config file',
    async () => {
  ...
  it('supports config that exports a function', async () => {
    ...
    assert.deepEqual(
      await requireWebpackConfig(configPath, false, 'test'),
      expectedConfig
    )
  })
  ...
  it('throws error when multi compiler config is given', async () => {
    ...
    await rejects(() => requireWebpackConfig(configPath, true), {
      message: error
    })
  })
