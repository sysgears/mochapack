...
describe('requireWebpackConfig', () => {
  const getConfigPath = (extension, suffix = 'config-test') => path.join(__dirname, 'fixture', 'webpackConfig', `webpack.${suffix}${extension}`);
  ...
  it('throws error when not found when required', async () => {
    ...
    await rejects(() => requireWebpackConfig(configPath, true), { message: `Webpack config could not be found: ${configPath}` });
  });
